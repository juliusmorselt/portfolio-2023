...
type ExperienceData = {
    _id: string
    _createdAt: string
    categories: any[] // You can replace 'any[]' with the actual type if possible
}
    ...
    <React.Fragment key={index}>
      <div className='flex flex-wrap sm:flex-nowrap justify-center gap-2 text-center pb-6'>
        {item.categories.map((category: any, categoryIndex: number) => {
          const { category: categoryName, companies } = category;
          ...
          return (
              ...
              >
                <p className={`mb-0 p-4 + ${ isActive ? "font-bold" : "font-regular" }`}>{categoryName}</p>
              </div>
          );
        })}
      </div>
      {item.categories.map((category: any, categoryIndex: number) => {
        ...
        return (
          <div key={categoryIndex}>
            ...
            {isActive && (
              <div className="py-6">
                <Timeline companies={companies} isActive={isActive} />
              </div>
            )}
          </div>
        );
